<template>
  <div>
    <div  v-if="searchable" class="flex gap-4 justify-between items-center py-4">
      ...
      <span class="inline-block whitespace-nowrap text-base font-semibold bg-gray-300 text-gray-900 rounded-md py-2 px-3"> Satellites: {{ rows.length }}</span>
    </div>
      ...
      <div class="-my-2 overflow-x-auto">
        <div class="py-2 align-middle inline-block min-w-full">
          <div class="shadow sm:rounded-lg overflow-hidden select-none">
            <div class="h-[75vh] overflow-auto relative" v-bind="containerProps">
              <div
              ...
              <div v-bind="wrapperProps" class="relative">
                <div
                  v-for="item in list"
                  :key="item.index"
                  :class="['grid px-6 py-4 text-sm text-white border-b border-slate-700']"
                  :style="`grid-template-columns: repeat(${columns.length || 1}, minmax(0, 1fr));`"
                >
...
import { MagnifyingGlassIcon, ChevronUpIcon, ChevronDownIcon, XMarkIcon } from '@heroicons/vue/24/solid'
